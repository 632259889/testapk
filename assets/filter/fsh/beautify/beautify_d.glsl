...
highp vec3 centralColor;

highp float mul_x = 1.6 / x_a;
highp float mul_y = 1.6 / y_a;
...
highp float aa = 1.0 + pow(sum, 0.3)*0.07;
highp vec3 smoothColor = centralColor*aa - vec3(sample)*(aa-1.0);// get smooth color
...
smoothColor = mix(centralColor, smoothColor, 0.75);
