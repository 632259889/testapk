...
const highp vec4 params = vec4(0.5, 0.63, 0.5, 0.1);
...
highp float hardLight(float color) {
    if(color <= 0.5) {
...
void main() {
    ...
    smoothColor.g = clamp(pow(smoothColor.g, params.g),0.0,1.1);
    smoothColor.b = clamp(pow(smoothColor.b, params.g),0.0,1.0);

    highp vec3 screen = vec3(1.0) - (vec3(1.0)-smoothColor) * (vec3(1.0)-centralColor);
    highp vec3 lighten = max(smoothColor, centralColor);
    highp vec3 softLight = 2.0 * centralColor*smoothColor + centralColor*centralColor
