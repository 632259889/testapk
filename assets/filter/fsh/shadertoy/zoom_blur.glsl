...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float distance = sqrt((fragCoord.x - center.x)*(fragCoord.x - center.x)+(fragCoord.y - center.y)*(fragCoord.y - center.y)*ratio*ratio);
    float s = 0.88;
    if(distance < radius){
        s = 0.0;
    }
    if(distance > radius && distance < radius + 0.1){
        s = s * (distance - radius)/0.1;
    }
    float Start = 2.0 / 14.0;
	vec2 Scale = s * 4.0 * 2.0 * PixelOffset.xy;
