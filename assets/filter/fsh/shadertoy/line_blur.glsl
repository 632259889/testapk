...
uniform highp float tiltHeight;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float distance = sqrt((fragCoord.y - center.y)*(fragCoord.y - center.y)*ratio*ratio);
    float s = 0.88;
    if(distance < tiltHeight/2.0){
        s = 0.0;
    }else{
        s = s * (distance- tiltHeight/2.0)/0.1;
    }
    float Start = 2.0 / 14.0;
    vec2 Scale = s * 4.0 * 0.5 * PixelOffset.xy;
